...
serve(async (req) => {
  ...
  try {
    ...
    // Apenas admin pode criar conta da Representada
    ...
    // Bloqueia criação se já existe alguém com role='representada' ativo.
    // (decisão de produto: conta única compartilhada por funcionários da Representada).
    ...
    if (existenteErr) {
      console.error('Erro ao verificar representada existente:', existenteErr)
      return new Response(JSON.stringify({ error: 'Erro interno do servidor' }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    ...
    if (existente && existente.length > 0) {
      return new Response(
        JSON.stringify({
          error: 'Já existe uma conta ativa da Representada. Desative-a antes de criar outra ou use o reset de senha.',
        }),
    ...
    if (profileError) {
      ...
      return new Response(JSON.stringify({ error: 'Erro ao criar perfil da representada' }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
